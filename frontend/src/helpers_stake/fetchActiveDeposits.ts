...
const fetchActiveDeposits = (options) => {
  const {
    address,
    chainId,
    offset = 0,
    ...
    const _doFetchBatch = () => {
      return new Promise((batchResolved, batchReject) => {
        callMulticall({
          ...
          encoder: abiI,
          calls: {
            batch: { func: 'getActiveDeposits', args: [ currentOffset, (limit < batchSize) ? limit : batchSize ], asArray: true },
          }
        }).then((answer) => {
          const {
            batch
...
export default fetchActiveDeposits