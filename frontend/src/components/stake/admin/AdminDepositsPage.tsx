...
const AdminDepositsPage = (props) => {
  ...
  const {
    gotoPage,
    params: {
      isActive: pageParamsIsActiveOnly
    },
    params
  } = props
  ...
  const [activeOnly, setActiveOnly] = useState((pageParamsIsActiveOnly == 'active'));
  ...
  useEffect(() => {
    setCurrentPage(1);
    setActiveOnly((pageParamsIsActiveOnly == 'active'))
  }, [ pageParamsIsActiveOnly ])
                ...
                <tbody className="divide-y divide-gray-800">
                  {deposits.map((deposit) => (
                    <tr key={deposit.depositId} className="hover:bg-gray-800">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm font-mono text-blue-300">
                        <a href={`#/admin/user/${deposit.owner}`}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm">
                        {(depositMonths[deposit.unlockMonthIndex]) ? (
                          <>{formatDate(depositMonths[deposit.unlockMonthIndex].start)}</>
                        ) : (
